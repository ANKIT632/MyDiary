...
const Form = ({ currentId, setCurrentId }) => {
  ...
  // after submit dispatch data 
  const handleSubmit = async (e) => {
    e.preventDefault();

    // new data the create new post.
    if (currentId === 0) {
  ...
  return (
    <Paper className={classes.paper}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        ...
        <TextField name="message" variant="outlined" label="Message" fullWidth multiline maxRows={4} value={postData.message} onChange={(e) => setPostData({ ...postData, message: e.target.value })} />
